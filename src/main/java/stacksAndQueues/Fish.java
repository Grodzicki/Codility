 ...
 * Then only one fish can stay alive − the larger fish eats the smaller one. More precisely, we say that two fish P and Q meet each other
...
public class Fish {
    ...
    public int solution(int[] A, int[] B){
        ...
        for (int j=i; j<B.length; j++){
            if (B[j]==1)
                myQueue.add(j);
            while (B[j]==0 && !myQueue.isEmpty()){
                if(A[j] > A[myQueue.peek()]){
                    myQueue.pop();
                    fish--;
                } else {
                    fish--;
